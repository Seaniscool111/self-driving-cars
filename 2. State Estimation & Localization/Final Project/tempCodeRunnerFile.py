...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    # h_jac is defined as [I_3x3 | 0_3x3 | 0_3x3] in the constants section
    ...
    # Orientation correction using axis-angle representation
    phi_hat = error_state[6:9]
    q_hat = Quaternion(axis_angle=phi_hat).quat_mult_left(q_check)
...
################################################################################################
p_cov[0] = np.eye(9) * 1e-10 
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    delta_t = imu_f.t[k] - imu_f.t[k - 1]

    # 1. Update state with IMU inputs
    # Get rotation matrix from the previous quaternion estimate
    C_ns = Quaternion(*q_est[k-1]).to_mat()
    
    # 1.1 Linearized Motion Model
    ...
    # Predicted orientation (Nominal state update)
    # delta_theta = imu_w * delta_t
